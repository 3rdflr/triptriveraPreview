...
export default function ActivityImageViewer({
  ...
  console.log('🖼️ [CSR] ActivityImageViewer 렌더링', {
    ...
    displayedImages: 3, // 배너 1개 + 서브 2개
    remainingCount,
  });

  return (
    <div className='w-full'>
      {/* 메인 이미지 섹션 - 첨부 사진처럼 배너 + 서브 2개만 표시 */}
      <div className='grid grid-cols-4 grid-rows-2 gap-2 md:gap-3 h-61 md:h-100'>
        ...
        <div className='col-span-2 row-span-2 relative rounded-s-3xl overflow-hidden bg-gray-100 group cursor-pointer'>
          ...
          <Image
            ...
            alt={title}
            fill
            className='object-cover transition-transform group-hover:scale-105 rounded-s-3xl'
            onClick={() => handleImageClick(0)}
            onLoad={() => handleImageLoad('main')}
            priority
          />
          {/* 호버 시 확대 아이콘 */}
          <div className='absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center rounded-s-3xl'>
